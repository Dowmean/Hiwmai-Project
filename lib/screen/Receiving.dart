  ...
  Future<void> fetchCompletedOrders() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('http://10.0.2.2:3000/getCompletedOrdersByEmail?email=$email'),
      );
...
ElevatedButton(
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => OrderReview(
          ref: order['order_ref'], // ใช้คีย์จากข้อมูล order
          email: order['order_email'], // อีเมลของผู้สั่งซื้อ
          productName: order['productName'], // ชื่อสินค้า
          productImage: order['product_image'], // รูปสินค้า
        ),
      ),
    );
  },
  ...
  child: Text("ให้คะแนน", style: TextStyle(fontSize: 16)),
),
